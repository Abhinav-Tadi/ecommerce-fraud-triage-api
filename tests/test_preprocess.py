"""
tests/test_preprocess.py — regression suite for scripts/preprocess.py's categorical encoding. 
Consolidates verify_categorical_fix.py, verify_schema_bool_passthrough.py, and find_collisions.py.
...
# Schema-level: JSON bool to model_dump()
...
def test_only_known_column_has_case_collisions():
    """
    DeviceInfo was the only column with a case collision. That fact is now hardcoded as _CASE_SENSITIVE_COLUMNS in preprocess.py. Nothing
    currently re-checks it against reality. If the model gets retrained and category_maps.json changes — DeviceInfo's collisions get cleaned up,
    or a different column develops one — this hardcoded set goes stale silently. This test makes that impossible: it re-derives the actual
    collision set from the live file, every run, and fails loudly the moment it disagrees with the hardcoded exclusion.
    """
...
def test_case_sensitive_column_bool_bridge_tries_both_cases(monkeypatch):
    """
    No column is both case-colliding and boolean-valued - the gap flagged when the case-sensitive exclusion shipped. Synthesizes
    that scenario via monkeypatch, using V1 (normally a plain numeric passthrough) as a stand-in, rather than waiting for some future
